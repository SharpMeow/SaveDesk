...
export function buildSharedPage(items) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>A Savedesk collection</title><style>
*{box-sizing:border-box}body{font-family:system-ui,sans-serif;background:#f7f8f3;color:#243b2c;max-width:1240px;margin:auto;padding:32px 24px}h1{font-size:36px;letter-spacing:-1px}header p{color:#536654}input{font:inherit;width:100%;padding:14px;border:1px solid #aabbab;border-radius:10px;margin:12px 0 24px}main{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,280px),1fr));gap:16px}article{background:#fff;border:1px solid #d9e1d6;border-radius:14px;padding:22px;overflow-wrap:anywhere}article p{white-space:pre-wrap;line-height:1.7}.author,.tags{font-size:13px;color:#536654}a{display:inline-block;color:#295638;margin-top:16px}footer{margin:32px 0;font-size:13px}button,input,a{outline-offset:4px}[hidden]{display:none!important}</style></head><body><header><small>SAVEDESK COLLECTION</small><h1>Ideas worth keeping.</h1><p>${items.length} saved posts. This is a shared snapshot, with no account or installation needed.</p><label for="search">Search this collection</label><input id="search" type="search" placeholder="Search words, authors, or topics…"><p id="count" role="status">${items.length} posts</p></header><main>${cards}</main><footer>Made with <a href="https://sharpmeow.github.io/savedesk/">Savedesk</a>. Post content belongs to its respective authors. Links open X; no content is fetched automatically.</footer><script>
